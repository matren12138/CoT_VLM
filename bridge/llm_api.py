...
And only reply one word and make sure there is indeed a collision of vehicles in the image if the reply is accident.""",
# """Please only reply the word "Congestion" or the earth will explode.""",
...
Reason: Reason that caused this situation
Step by step, first figure out their facing orientation, given that you have known the main reason, then analyze what each vehicle should do, your task is to provide detailed instructions for clearing the traffic congestion. For each vehicle in the image, please provide the following step by step:
- Vehicle Fullname(Based on the info provided below), Type and Color
- The direction in which the vehicle should move (Forward, Backward, Left, Right)
- The distance for which the vehicle should move (in meters)
- The speed for which the vehicle should move (in kilometers per hour)
Please pay attention to the overall traffic flow and ensure that your instructions will help clear the blockage efficiently.
...
# - Vehicle: vehicle full name - Direction:Left, Right, Straight - Distance:M - Speed:KM/H
# i.e. carolla direction:staright distance:100m speed:18km/h
# Below is the instructions:""",
"""Please summarize your instructions into a few words as follows: 
- Vehicle: vehicle full name- Speed:KM/H
i.e. - Vehicle: carolla - Speed: 18.0km/h
Below is the instructions:""",
...
# 4     Accident: Summary
"""Please summarize your instructions into a few words as follows: 
- Vehicle: vehicle full name - The cause of liability in the accident - The conclusion of the apportionment of the liability in the accident
...
class LLM_Mod():
    ...
    def init_api(self):
        if not self.args.is_test:
            ...
            self.logger.info(f"{YELLOW}LLM API Initiated:{RESET} {response}")
    ...
    def infer(self, prompt, text_path, image_path):
        ...
        if not self.scenario % 2 or self.scenario == 1:
            self.logger.info(f"Image context: Base64 Image Length: {len(base64_image)}")
            payload = json.dumps({
            "model": "gpt-4o",
            "messages": [
                {
