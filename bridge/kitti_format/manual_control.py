...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            location = carla.Location(-28.2, -93.9, 0.5)
...
def vehicle_npc_create(sim_world):
        ...
        # vehicle_list = create_circle_npc(blueprint_library, sim_world)

        # car_1
        # for i in range(5):
        #     location = carla.Location(-4.9, 8.0 - i * 6, 0.5)
        ...
        # for i in range(8):
        #     location = carla.Location(5.3, -36.0 - i * 6, 0.5)
        #     rotation = carla.Rotation(0, 270 ,0)
        #     spawn_point = carla.libcarla.Transform(location,rotation)
        #     vehicle_bp = blueprint_library.find('vehicle.audi.a2')
...
def tick(world, actors, data):
        ...
        for agent, dataQue in data["sensor_data"].items():
            data = [_retrieve_data(q) for q in dataQue]
...
def spawn_actors(world, client, actors):
        num_of_vehicles = 0
        num_of_walkers = 0
...
def game_loop(args):
    ...
    try:
        ...
        spawn_actors(sim_world, client, actors)
        ...
        while True:
            # data = tick(sim_world, actors, data)
            if args.sync:
                sim_world.tick()
