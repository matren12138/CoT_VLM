...
class BehaviorAgent(BasicAgent):
    ...
    def get_speed_limit_rtc(self):
        return self._vehicle.get_speed_limit()
    
    def get_speed_rtc(self):
    ...
    def _slow_moving_vehicle_ahead(self, waypoint, vehicle_list):
        """
        This method is in charge of detecting slow-moving vehicles ahead and performing lane change when necessary.
        :param waypoint: current waypoint of the agent
        :param vehicle_list: list of all the nearby vehicles
        """
        ...
        left_wpt = waypoint.get_left_lane()
        right_wpt = waypoint.get_right_lane()
        print(f"Now {self._local_planner.target_waypoint.transform.location}    Set {right_wpt.transform.location}")
        self.set_destination(right_wpt.transform.location)
        front_vehicle_state, front_vehicle, _ = self._vehicle_obstacle_detected(
            vehicle_list, max(self._behavior.min_proximity_threshold, self._speed_limit / 2), up_angle_th=90, low_angle_th=70)
        if front_vehicle_state is not False:
            #print(f"Front Vehicle State {front_vehicle_state}")
            print(f"Front Vehicles {front_vehicle}")
            print(f"My speed {self._speed}, Former speed {get_speed(front_vehicle)}")
        if front_vehicle_state and self._speed > get_speed(front_vehicle):
            ...
            waypoint.lane_id * right_wpt.lane_id > 0 and right_wpt.lane_type == carla.LaneType.Driving:
                ...
                if new_vehicle_state:
                    print("Low speed ahead, moving to the right!")
                    end_waypoint = self._local_planner.target_waypoint
                    self._behavior.tailgate_counter = 200
                ...
                new_vehicle_state, _, _ = self._vehicle_obstacle_detected(vehicle_list, max(self._behavior.min_proximity_threshold, self._speed_limit / 2), up_angle_th=180, lane_offset=-1)
                if not new_vehicle_state:
                    print("Low speed ahead, moving to the left!")
                    end_waypoint = self._local_planner.target_waypoint
                    self._behavior.tailgate_counter = 200
    ...
    def collision_and_car_avoid_manager(self, waypoint):
        ...
        if self._direction == RoadOption.CHANGELANELEFT:
            ...
            # Check for lanechange
            #self._slow_moving_vehicle_ahead(waypoint, vehicle_list)
